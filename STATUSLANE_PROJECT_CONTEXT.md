...
- Sync triggers: GitHub Action `.github/workflows/cron-sync.yml` runs every minute and calls `/api/ics-sync-run` (ICS still relies on polling). Google/Outlook transitions are now handled by Cloud Tasks scheduled inside the sync flows, while `.github/workflows/watch-refresh.yml` (hourly) refreshes Google + Outlook watch tokens. Requires repo secrets `SYNC_BASE_URL` (e.g., https://statuslanes.vercel.app) and `SYNC_SECRET`. Vercel cron removed. Heartbeat removed from dashboard; rely on Actions + manual sync (or Cloud Tasks).
- Local heartbeat: you can run `npm run heartbeat` (script at `scripts/calendar-heartbeat.js`) on any persistent server/container. It reads `SYNC_BASE_URL`, `SYNC_SECRET`, and optional `HEARTBEAT_INTERVAL_MS` to POST `/api/calendar-cache/apply` on a short interval so statuses flip immediately when meetings end, without waiting on GitHub Actions.
- TRMNL pushes: Google/ICS bulk and self sync push status to TRMNL when status changes.
- Env vars: `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GOOGLE_REDIRECT_URI`, `GOOGLE_CALENDAR_WEBHOOK_URL`, `OUTLOOK_CLIENT_ID`, `OUTLOOK_CLIENT_SECRET`, `OUTLOOK_REDIRECT_URI`, `OUTLOOK_TENANT_ID`, `OUTLOOK_CALENDAR_WEBHOOK_URL`, `SYNC_SECRET`, Firebase creds, `WEBHOOK_SECRET_KEY`, `NEXT_PUBLIC_APP_URL`, etc. Provider secrets only in Vercel/.env.local (not in GitHub). Outlook client secret currently set with a 72-month expiration; schedule rotation before it lapses.
- Google push watches: `GOOGLE_CALENDAR_WEBHOOK_URL` points to `/api/google-calendar/webhook`. We store watch metadata per calendar in `google_calendar_channels` (channelId/resourceId/deviceId/userId/expiration/token). Device PATCH ensures watches whenever `calendarIds` change, cron `watch-refresh` job keeps them renewed by calling `/api/google-calendar/watch-refresh`, disconnect removes all channels, and the webhook triggers `runGoogleSyncForUser` immediately on notifications.
- Outlook (preview): Users can connect/disconnect Microsoft (multi-tenant) via `/api/outlook-calendar/{auth,callback,status,calendars,disconnect}`. Settings show Google and Outlook blocks side-by-side so users can enable any combination of calendars at once (per-provider lists + manual sync buttons). `/api/outlook-calendar/sync` fetches selected calendars via Microsoft Graph, maps events with the shared `calendarSync` helpers, and pushes TRMNL updates. Outlook tokens live in `outlook_tokens` with `lastSyncedAt`. Webhooks: `/api/outlook-calendar/webhook` receives Graph notifications, backed by Firestore collection `outlook_subscriptions`. Device PATCH ensures subscriptions via `ensureOutlookSubscriptionsForDevice`, `watch-refresh` workflow also calls `/api/outlook-calendar/watch-refresh` hourly, and disconnect removes all subscriptions.
- Outlook integration WIP: backend scaffolding (`src/lib/outlook*.ts`, `/api/outlook-calendar/{auth,callback,status,calendars,disconnect}`) handles OAuth/token storage via Microsoft Graph. Manual sync/webhook endpoints + device settings UI are the next steps.
- Automation webhook (IFTTT/MacroDroid/Shortcuts): `POST /api/automation/geofence` with header `x-automation-secret: <device.automationSecret>` sets device status using `automationId` (per-device token), required `statusKey` (1-12), optional `statusLabel` (otherwise fetched from device), and optional `statusSource` (defaults to "Automation"); updates TRMNL immediately. `automationId` and `automationSecret` are generated on device creation (or backfilled on next update if missing).
