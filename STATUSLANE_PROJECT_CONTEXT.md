...
- Outlook (preview): Users can connect/disconnect Microsoft (multi-tenant) via `/api/outlook-calendar/{auth,callback,status,calendars,disconnect}`. Settings now support provider selection (Google, Outlook, ICS), per-provider calendar lists, and manual sync buttons. `/api/outlook-calendar/sync` fetches selected calendars via Microsoft Graph, maps events with the shared `calendarSync` helpers, and pushes TRMNL updates. Outlook tokens live in `outlook_tokens` with `lastSyncedAt`. Outlook webhooks/subscription renewal are TODO; for now users trigger manual sync or future scheduled jobs will hit the endpoint. 
