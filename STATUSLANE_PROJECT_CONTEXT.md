...
- Sync triggers: GitHub Action `.github/workflows/cron-sync.yml` runs every minute to hit `/api/ics-sync-run`, while `.github/workflows/cache-advance.yml` (same cadence) POSTs to `/api/calendar-cache/apply` so cached Google/Outlook/ICS events flip statuses at the right timestamps without calling provider APIs. `.github/workflows/watch-refresh.yml` (hourly) calls `/api/google-calendar/watch-refresh` and `/api/outlook-calendar/watch-refresh`. Requires repo secrets `SYNC_BASE_URL` (e.g., https://statuslanes.vercel.app) and `SYNC_SECRET`. Vercel cron removed. Heartbeat removed from dashboard; rely on Actions + manual sync.
