...
- Sync triggers: GitHub Action `.github/workflows/cron-sync.yml` runs every minute, calling `/api/ics-sync-run` then `/api/calendar-cache/apply` (for cached Google/Outlook/ICS events) with the shared `x-sync-secret`. `.github/workflows/watch-refresh.yml` (hourly) calls `/api/google-calendar/watch-refresh` and `/api/outlook-calendar/watch-refresh`. Requires repo secrets `SYNC_BASE_URL` (e.g., https://statuslanes.vercel.app) and `SYNC_SECRET`. Vercel cron removed. Heartbeat removed from dashboard; rely on Actions + manual sync.
