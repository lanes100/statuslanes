...
- Automation webhook (IFTTT/MacroDroid/Shortcuts): `POST /api/ifttt/geofence` with header `x-ifttt-secret: <device.iftttSecret>` (or bearer) sets device status using `iftttId` (per-device token), required `statusKey` (1-12), optional `statusLabel` (otherwise fetched from device), and optional `statusSource` (defaults to "IFTTT Geofence"); updates TRMNL immediately. `iftttId` and `iftttSecret` are generated on device creation (or backfilled on next update if missing).
