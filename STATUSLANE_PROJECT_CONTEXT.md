...
- Sync triggers: GitHub Action `.github/workflows/cron-sync.yml` every 15 min hitting `/api/ics-sync-run` secured by `x-sync-secret`. Separate `.github/workflows/watch-refresh.yml` (hourly) calls `/api/google-calendar/watch-refresh` to renew push channels (and will later cover Outlook). Requires repo secrets `SYNC_BASE_URL` (e.g., https://statuslanes.vercel.app) and `SYNC_SECRET`. Vercel cron removed. Heartbeat removed from dashboard; rely on Action + manual sync.
